...
std::string execute_command(const std::string &cmd, const std::string &cwd) {
  int pipefd[2];
  // tạo pipe
  ...
  if (pid == 0) {     // Child process
    ...
    if (chdir(cwd.c_str()) == -1) {
      std::cerr << "Error: chdir failed to " << cwd << std::endl;
      exit(1);
    }
...
void *handle_client(void *arg) {
  ...
  std::cout << "[NEW CONNECTION] Client connected." << std::endl;
...
int main() {
  ...
  std::cout << "[LISTENING] Server is listening on port " << PORT << std::endl;
